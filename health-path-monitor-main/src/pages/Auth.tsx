  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // If patient, trigger Flask simulation to start pushing demo data
        if (role === "patient" && uid) {
          try {
            const res = await fetch("http://127.0.0.1:5000/start_simulation", {
              method: "POST",
            ...
            if (res.ok) toast.success("Simulation started (backend).");
            else toast.warning("Simulation not started. Check Flask.");
          } catch (err) {
            console.warn("Flask trigger failed", err);
            toast.warning("Could not connect to simulation server.");
          }
        }

        // allow auth observer to populate user before navigate
        setTimeout(() => navigate("/dashboard"), 700);
      }
    } catch (err: any) {
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="p-8 shadow-custom-lg bg-slate-900/60 backdrop-blur">
          <div className="mb-6 text-center">
            ...
            <p className="text-muted-foreground mt-1">{isLogin ? "Login to access your dashboard" : "Join as doctor or patient"}</p>
          </div>
            ...
            {!isLogin && (
              <>
                ...
                <div className="space-y-2">
                  <Label className="text-foreground">Select Role</Label>
                  <div className="flex gap-4">
                    <button type="button" onClick={() => setRole("doctor")} className={`flex-1 flex items-center justify-center gap-2 border rounded-lg py-2 ${role === "doctor" ? "border-blue-500 bg-blue-500/20 text-blue-400" : "border-gray-700 hover:bg-gray-800"}`}>
                      <Stethoscope className="h-5 w-5" /> Doctor
                    </button>
                    <button type="button" onClick={() => setRole("patient")} className={`flex-1 flex items-center justify-center gap-2 border rounded-lg py-2 ${role === "patient" ? "border-green-500 bg-green-500/20 text-green-400" : "border-gray-700 hover:bg-gray-800"}`}>
                      <UserCircle className="h-5 w-5" /> Patient
                    </button>
                  </div>
                </div>
